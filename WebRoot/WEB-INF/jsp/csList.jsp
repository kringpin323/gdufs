...
<html>
  <head>
    ...
    function labTable(e,d){
     $.post('paike!labTab.action',
     {'labID':e,'cdate':d},
     function(data){
   $('#tt').html(data);
   ...
   function toExcel(e,d,w){
 	var IP=$('#IP').val();
   window.top.location = encodeURI("http://"+IP+"/<%=request.getContextPath()%>/paike!toExcel?labID="+e+"&cdate="+d+"&week="+w);
...
return;
	...
	'c.class_':$('#class').val(),
	'c.major':$('#major').val(),
	'c.closeDoorDelay':$('#CloseDoorDelay').val(),
	'c.openDoorAdvance':$('#OpenDoorAdvance').val()
	},
        function(data){
        if(data=='Y'){
       alert(" 添加成功");
...
var week;
	function toChangWeeks(){
	 week=$('#toChangWeeks').val();
		var startD=$('#startD').val();
		
		var days=(week-1)*7;
	var sureDate=doit(startD,days);
	var labId=$('#everyLabId').val();
	
		

	labTable(labId,sureDate);
	
	
	//为了达到休眠的效果 执行一段没有实际意义的代码
	
	 setTimeout(function stop(){

	
	var toChangWeeks=document.getElementById('toChangWeeks');
		var ops=toChangWeeks.options;
	...
	var week=$('#changWeeksEnd').val();
		var startD=$('#startD').val();
		var day= parseInt($('#weekNumEnd').val());
		var days=(week)*7+day;
	var sureDate=doit(startD,days);
  ...
  <tr >
    <td >
     <%
        ...
        java.sql.Date   cdate=new   java.sql.Date(dates.getTime()); 
     %>
    <s:iterator value="laList" var="l">
    <input type="button" id="chaxun" onclick="labTable('${l.labId}','<%=cdate%>')" value="${l.labDesc}"/>
    </s:iterator>
...
<div id="tt"></div>
  ...
  <table  border="0" cellspacing="0">
  	<tr>
  		<td>实验室名称：</td>
  		<td>
  		<select id="LabID" style="width:165px">
  		 <s:iterator value="laList" var="l">
  	...
  	<tr>
  		<td>时　间　段：</td>
  		<td>
  			   <select id="ClasshourID" style="width:165px">
                    <s:iterator value="ccList" var="c" status="st">
                    <option value="${c.classhourId}">${c.classhourName}  (第${st.count}节课)</option> 
                    </s:iterator>
               </select>
  		</td>
  	</tr>
  	...
  	<tr>
  		<td>是否重复执行：</td>
  		<td>
  			<select id="isRepetitive" style="width:165px" onchange="fun2()">
  	...
  	<tr>
  		<td>循环类型：</td>
  		<td>
  			<select id="cycleType" style="width:165px" onchange="fun3()">
  				<option ></option>
...
<select id="weekNum"  onchange="changWeeks()">
...
<select id="weekNumEnd"  onchange="changWeeksEnd()">
<option value="-1">星期日</option>
<option value="0">星期一</option>
<option value="1">星期二</option>
<option value="2">星期三</option>
...
<option value="5">星期六</option>
  		...
  		<tr><td>提前开门时间：</td><td><input type="text" id="OpenDoorAdvance"> 分钟</td></tr>
  		<tr><td>推迟关门时间：</td><td><input type="text" id="CloseDoorDelay"> 分钟</td></tr>
  	<tr>
