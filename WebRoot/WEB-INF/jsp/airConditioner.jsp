...
<!--
body {
	background-image: url(image/airConditioner/bg.png);background-repeat: no-repeat;
}
...
function lightControl(o,l,d){
  var url = "lab_startConditionerControl?link="+l+"&action="+o+"&deviceId="+d;
...
function setResult(o){
   for(var i=0;i<o.length;i++){
    if(null!=(document.getElementById("light"+(o[i][0])))){
     if(o[i][1]=="on"){ document.getElementById("light"+(o[i][0])).innerText = "关闭";} else{document.getElementById("light"+(o[i][0])).innerText = "打开";}
    }
    }
  }
...
<tr>
  <td height="26" colspan="5" style="height: 33px;width: 1002px;background:url(image/airConditioner/navigation.png);margin: 0px;padding: 0px;"></td>
</tr>

<tr><td colspan="5"><DIV class="nav">
  <UL>
  <s:iterator value="labInfos" var="lab_Info">
    <LI><A href="javascript:lightList('${lab_Info.labId}','空调')" >${lab_Info.labDesc}</A></LI>
   </s:iterator>  
  </UL>
</DIV></td></tr>
