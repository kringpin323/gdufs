...
System.out.println(sql);
	...
	public void addLabRight(String labID, String userid) {
		Session session=HibernateUtils.getSession();
		String sql="insert into lab_right(LabID,UserID,GroupID) values('"+labID+"','"+userid+"','******')";
		session.createSQLQuery(sql).executeUpdate();
		session.beginTransaction().commit();
		HibernateUtils.close();
	...
	public String getLabIDByExec(String userid) {
		...
		CallableStatement cstmt;
		try {
			cstmt = conn.prepareCall("exec dbo.upd_CampusNetUserLogin '"+userid+"'");
			ResultSet rs= cstmt.executeQuery(); 
		if(rs.next()){
			return rs.getString("Lab");
		}
	...
	public String getLabIdByDoorNum(String doorNum) {
		String sql="select labID from session_parameter where SessionValue='"+doorNum+"'";
		Session session=HibernateUtils.getSession();
		Connection conn=session.connection();
		try {
			PreparedStatement stat=conn.prepareStatement(sql);
